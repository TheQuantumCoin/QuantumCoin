...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Quantumcoin: starting difficulty is 1 / 2^12
...
{
    // Quantumcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Bonus reward for block 10,000 - 19,999 of 450,000 coins
...
// Bonus reward for block 2-9,999 of 600,000 coins
...
static const int64 nTargetTimespan = 60 * 60; // Quantumcoin: Not used
static const int64 nTargetSpacing = 60; // Quantumcoin: 1 minute block
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Quantumcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Quantumcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Quantumcoin: increase each by adding 2 to bitcoin's value.
...
void static QuantumcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running QuantumcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QuantumcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QuantumcoinMiner, pwallet));
}
